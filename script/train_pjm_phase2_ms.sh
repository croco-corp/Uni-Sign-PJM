#!/bin/bash
# Faza 2 - LoRA fine-tune na PJM multi-speaker split (random, 24k train).
...
output_dir=out/pjm_phase2_lora_ms
mkdir -p "$output_dir"

deepspeed --include localhost:0 --master_port 29512 fine_tuning.py \
  --batch-size 16 \
